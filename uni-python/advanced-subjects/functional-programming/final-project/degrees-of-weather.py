...
if st.session_state.page == "Pogoda":
    ...
    st.warning("Kraków charakteryzuje się umiarkowanym klimatem przejściowym, w którym historycznie (na przestrzeni ostatniego stulecia) obserwuje się wzrost średnich temperatur oraz wyraźne zróżnicowanie jakości powietrza.")

    daily_data_temp = get_daily_averages(data, 'temp')
    ...
    with col2:
        ...
        st.text("i tak powstaje smog: Mgła (wysoka wilgotność) w połączeniu z zanieczyszczeniami tworzy aerozol, który znacznie trudniej się rozprasza niż suchy pył.")
...
if st.session_state.page == "PM10":
    ...
    with col1:
        ...
        viewdata = round(smog_ep["air_quality"].groupby(smog_ep["air_quality"]).count().sort_values(ascending=False)/24, 2)
    ...
    with col2:
        ...
        st.dataframe(smog_ep["worst_pollution"])
...
if st.session_state.page == "CO2":
    ...
    daily_data_no2 = get_daily_averages(data, 'no2')
    ...
    st.info("Zauważ dwa piki: poranny (7-9) i popołudniowy (16-18). To efekt rur wydechowych.")
